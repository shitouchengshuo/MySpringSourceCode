...
public class DispatcherServlet extends FrameworkServlet {
	...
	static {
		...
		try {  // 去DispatcherServlet所在的类路径下(resources文件下DispatcherServlet.properties文件)找一个DispatcherServlet
	...
	@Nullable // handler(处理器、能处理请求的人(Controller))的映射,保存请求与处理的方法的映射关系
	...
	@Nullable  // 把请求转成视图名(我们要跳转的也页面地址)的翻译器【没啥用，非8大】
	 ...
	 */ // 8大组件初始化，spring容器刷新完成后发送事件，利用事件监听器回调到该方法
	protected void initStrategies(ApplicationContext context) {
		initMultipartResolver(context); // 下面的都是从容器中取， 没有就取配置文件中默认的
	...
	protected Object createDefaultStrategy(ApplicationContext context, Class<?> clazz) {
		return context.getAutowireCapableBeanFactory().createBean(clazz);// 启动ioc容器创建bean
	}
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		HandlerExecutionChain mappedHandler = null;  // 目标方法的执行链
		boolean multipartRequestParsed = false;
        // 对异步请求的支持（servlet 3.0以后才有,webflux）
		...
		try {
			...
			try {//文件上传相关，检查是否是文件上传请求
				processedRequest = checkMultipart(request);
				multipartRequestParsed = (processedRequest != request);

				//  构造出了【目标方法+拦截器整个链路】决定使用哪个Handler处理当前的请求， Determine handler for the current request.
				...
				// 重点: 适配器怎么找的   Determine handler adapter for the current request.
				...
				// 核心：真正执行目标方法  Actually invoke the handler.
